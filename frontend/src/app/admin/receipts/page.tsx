...
const ReceiptsPage: React.FC = () => {
  ...
  const fetchReceipts = async () => {
    try {
      setLoading(true)
      setError('')
      const receiptsData = await adminApiClient.getReceipts()
      // Transform the API data to match our Receipt interface
      const transformedReceipts: Receipt[] = receiptsData.receipts.map(receipt => ({
        id: receipt.id,
        order_id: receipt.order_number,
        customer_name: receipt.customer_name,
        date: receipt.created_at,
        total: receipt.total_amount,
        payment_status: 'COMPLETED' as const, // API doesn't provide this, assuming completed
        receipt_url: `/receipts/${receipt.id}.pdf`,
      }))
  ...
  const downloadReceipt = async (orderId: string) => {
    try {
      await adminApiClient.downloadReceipt(orderId)
    } catch (err: unknown) {
      setError(handleApiError(err))
  ...
  const handleDownloadReceipt = async (receipt: Receipt) => {
    await downloadReceipt(receipt.order_id)
  }
